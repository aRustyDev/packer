...
source "proxmox-iso" "proxmox-ubuntu-21" {
  proxmox_url      = "https://192.168.0.42:8006/api2/json"
  vm_name          = "packer-ubuntu-21"
  iso_url          = "http://192.168.0.144/ubuntu-21.10-live-server-amd64.iso"
  ...
  boot_command = [
    ...
    " autoinstall ds=nocloud-net;s=http://192.168.0.144:80/preseed/ubuntu-21/ ", #Dont specify the file
    "boot",
    "--- <enter>"
  ]
  ...
  template_name        = "packer-ubuntu-21"
  template_description = "packer generated ubuntu-21.10-server-amd64"
  unmount_iso          = true
...
build {
  sources = ["source.proxmox-iso.proxmox-ubuntu-21"]
  provisioner "shell" {
    inline = [
